...
class Env(object):
    def __init__(self, name, max_step, max_reward=None):
        self.name = name
    ...
    def show(self, nn, interval=None):
        while True:
            s = self.f.reset()
...
def learning():
    ...
    for gen in range(MAXGEN):
        ...
        print('Gen: ', gen,
              ...
              ' net_cr: %.3f' % net_cr,
              ' t: %.3f' % (te - ts))
        if env.max_reward is not None and net_r > env.max_reward:
            break
